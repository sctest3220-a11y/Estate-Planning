...
TRANSLATIONS = {
    ...
    "q_download_template": {"en": "⬇ Download Excel template", "th": "⬇ ดาวน์โหลดแม่แบบ Excel"},
    "q_upload_csv": {"en": "⬆ Upload filled sheet", "th": "⬆ อัปโหลดไฟล์ที่กรอกแล้ว"},
    "q_upload_hint": {
        "en": "The Excel template has a tab for each asset category with extra fields; required columns are marked *. Fill in the tabs that apply and upload it here (a .csv also works). The file is imported when you submit this form. You can also type assets directly below.",
        "th": "แม่แบบ Excel มีแท็บสำหรับทรัพย์สินแต่ละประเภทพร้อมช่องรายละเอียดเพิ่มเติม คอลัมน์ที่บังคับมีเครื่องหมาย * กรอกแท็บที่เกี่ยวข้องแล้วอัปโหลดที่นี่ (ไฟล์ .csv ก็ใช้ได้) ไฟล์จะถูกนำเข้าเมื่อส่งแบบฟอร์ม หรือจะพิมพ์ทรัพย์สินด้านล่างโดยตรงก็ได้",
    },
